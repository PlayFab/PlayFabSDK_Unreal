...
public class OnlineSubsystemPlayFab : ModuleRules
{
	...
	private static class GDKExports
	{
		...
		static GDKExports()
		{
			...
			else
			{
				...
				if (GetCurrentGSDKDirMethodInfo == null)
				{
					throw new Exception("Faild to locate GetCurrentGSDKDir");
				}
			}
		}
		...
		{
			return GetCurrentGSDKDirMethodInfo != null ? (string)GetCurrentGSDKDirMethodInfo.Invoke(null, null) : string.Empty;
		}
	}
	...
	private void ConfigureForGDKPlatform()
	{
		...
		PublicAdditionalLibraries.Add(Path.Combine(LibPath, "PlayFabMultiplayerGDK.lib"));
		...
		RuntimeDependencies.Add("$(TargetOutputDir)/PlayFabMultiplayerGDK.dll", Path.Combine(BinPath, "PlayFabMultiplayerGDK.dll"), StagedFileType.SystemNonUFS);
		RuntimeDependencies.Add("$(TargetOutputDir)/PlayFabMultiplayerGDK.pdb", Path.Combine(BinPath, "PlayFabMultiplayerGDK.pdb"), StagedFileType.DebugNonUFS);

		System.Console.WriteLine("copy Party.dll from {0}: TargetOutputDir", BinPath);
