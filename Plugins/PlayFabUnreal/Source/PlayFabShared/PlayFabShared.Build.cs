...
public class PlayFabShared : ModuleRules
{
    ...
    public PlayFabShared(ReadOnlyTargetRules Target) : base(Target)
    {
        LogPlayFabShared($"Loading module for target: {Target.Name}, Platform: {Target.Platform}, Configuration: {Target.Configuration}");
        ...
        LogPlayFabShared($"Added dependencies: {string.Join(", ", PublicDependencyModuleNames)}");
    ...
    private void ConfigureForWindowsPlatform()
    {
        ...
        int gdkVersion = PlayFabGDKUtilities.GetGDKVersionWithFallback("PlayFabShared");

        // Verify the version meets requirements
        if (gdkVersion < 251000)
        {
            throw new InvalidOperationException($"GDK version {gdkVersion} is too old. Version 251000 or higher is required.");
        }
